...
bool HelloWorld::init() {
	...
	logger().debug() << "test HelloWorld::createScene()" << logs::end;
	...
	//preparation des parametres pour le test de led
	char *ptest[] = { "LegoEV3", "t", "_n", "1", "_s", "_c", "green", NULL };
	robotlegoev3.parseConsoleArgs(7, ptest);

	//launch automate
	robotlegoev3.begin(7, ptest);

	logger().debug() << "End threadLegoEV3RobotExtended " << logs::end;
