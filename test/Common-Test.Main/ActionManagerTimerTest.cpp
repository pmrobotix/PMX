...
public:
	...
	bool execute()
	{
		...
		logger().info() << "executing " << name_ << "... t=" << time - lasttime_ << "us"
				<< logs::end;
		lasttime_ = time;
		nb_++;
		if (nb_ < nbmax_)
...
public:
	...
	MockTimer(std::string label, test::ActionManagerTimerTest & actionManagerTimerTest,
			int timeSpan_ms) :
...
void test::ActionManagerTimerTest::testCount()
{
	...
	this->assert(manager.countActions() == 1,
			"ActionManagerTimer::countActions() ne renvoie pas 1 après un ajout");

	manager.debugActions();

	MockTimer *timer = new MockTimer("MockTimer1", *this, 100);
	manager.addTimer(timer);
	this->assert(manager.countTimers() == 1,
			"ActionManagerTimer::countTimers() ne renvoie pas 1 après un ajout");
	...
	manager.clearTimers();
	this->assert(manager.countTimers() == 0,
			"ActionManagerTimer::countTimers() ne renvoie pas 0 après clearTimers()");
	delete timer;
