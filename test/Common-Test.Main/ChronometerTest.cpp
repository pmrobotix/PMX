...
void test::ChronometerTest::testTimerNanosleep()
{
    ...
    //printf( "\n",res.tv_nsec / 1000 );
    logger().info() << "clock_getres() : Resolution minimum is " << res.tv_nsec / 1000 << " micro seconds."
            << logs::end;

    logger().info() << " testTimerNanosleep : Comparaison d'attente avec nanosleep/usleep" << logs::end;
...
void test::ChronometerTest::testWithClock()
{
    ...
    logger().info() << "testWithClock: Resolution minimum avec fct Clock():"
            << (double) (t2 - t1) / CLOCKS_PER_SEC * 1000 << " ms." << logs::end;

    logger().info() << "CLOCKS_PER_SEC: " << CLOCKS_PER_SEC << " ; CLOCK() : " << clock() << logs::end;
}
...
void test::ChronometerTest::testWithGettimeofday()
{
    ...
    logger().info() << "testWithGettimeofday: Resolution minimum avec fct gettimeofday(): " << elapsedTime << " ms."
            << logs::end;
    ...
    elapsedTime = (t2.tv_sec - t1.tv_sec) * 1000.0; // sec to ms
    elapsedTime += (t2.tv_usec - t1.tv_usec) / 1000.0; // us to ms
    logger().info() << ":testWithGettimeofday: Resolution 1000us avec fct gettimeofday(): " << elapsedTime << " ms."
            << logs::end;
}
...
void test::ChronometerTest::testTimer()
{
    ...
    logger().info() << "Wait 1000us with usleep(): " << chrono.getElapsedTimeInSec() << " s. "
            << chrono.getElapsedTimeInMilliSec() << " ms. " << chrono.getElapsedTimeInMicroSec() << " us. "
            << logs::end;

    chrono.start();
    usleep(10000);
    chrono.stop();
    logger().info() << "Wait 10000us with usleep(): " << chrono.getElapsedTimeInSec() << " s. "
            << chrono.getElapsedTimeInMilliSec() << " ms. " << chrono.getElapsedTimeInMicroSec() << " us. "
            << logs::end;
    ...
    usleep(75000);
    chrono.stop();
    logger().info() << "Wait 75000us with usleep(): " << chrono.getElapsedTimeInSec() << " s. "
            << chrono.getElapsedTimeInMilliSec() << " ms. " << chrono.getElapsedTimeInMicroSec() << " us." << logs::end;
...
void test::ChronometerTest::testTimerTickMin()
{
    ...
    logger().info() << oss.str() << " (en us) sum=" << us << " nbtot=" << nb << logs::end;
}
