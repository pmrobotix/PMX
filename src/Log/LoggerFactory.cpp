...
logs::LoggerFactory::logger(const std::string & name)
{
	...
	std::map<std::string, logs::Logger *>::iterator value =
			instance.loggers_.find(name);
...
logs::LoggerFactory::appender(const std::string & name)
{
	...
	std::map<std::string, logs::Appender *>::iterator value =
			this->appenders_.find(name);
...
void logs::LoggerFactory::add(const std::string & name,
...
void logs::LoggerFactory::add(const Level & level,
		const std::string & loggerName, const std::string & appenderName)
{
