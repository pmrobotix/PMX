...
public:
	...
	void startTimer() {
		if (!started_) {
			...
			if (timer_create(CLOCK_REALTIME, &this->signalEvent, &this->timerID) != 0) { // timer id koennte mit private probleme geben
				std::cout << "ERROR ITimerPosixListener Could not create the timer name:"
						<< this->name() << std::endl;
			...
			if (sigaction(SIGALRM, &this->SignalAction, NULL)) {
				std::cout << "ERROR ITimerPosixListener Could not install new signal handler, id:"
						<< this->timerID << " name:" << this->name() << std::endl;
			...
			if (this->timerID != 0) {
				if (timer_settime(this->timerID, 0, &this->timerSpecs, NULL) == -1) {
					...
					std::cout << "ERROR ITimerPosixListener Could not start timer, id:"
							<< this->timerID << " name:" << this->name() << std::endl;
					perror("Could not start timer!");
					exit(1);
				}
			} else {
				std::cout << "ERROR ITimerPosixListener timerID is 0, id:" << this->timerID
			...
			std::cout << "ERROR ITimerPosixListener timer already started, id:" << this->timerID
					<< " name:" << this->name() << std::endl;
		}
	}
	...
	void remove(timer_t thistimerID) {
		...
		if (timer_delete(thistimerID) != 0) { // timer id koennte mit private probleme geben
			std::cout << "ERROR ITimerPosixListener remove - Could not delete the timerID: "
					<< thistimerID << std::endl;
...
// get the pointer out of the siginfo structure and asign it to a new pointer variable
		ITimerPosixListener *ptrTimerPosix =
				reinterpret_cast<ITimerPosixListener*>(si->si_value.sival_ptr);
