...
Asserv::Asserv(std::string botId, Robot *robot) {
    ...
    //init des objets
    if (useAsservType_ == ASSERV_INT_INSA) pAsservInsa_ = new AsservInsa(robot);
    else pAsservInsa_ = NULL;

    if (useAsservType_ == ASSERV_INT_ESIALR) pAsservEsialR_ = new AsservEsialR(robot); //TODO necessaire pour svg, botcodeur, et motors, essayer de donner uniquement l'asserv avec ce qu'il faut... a reflechir
    else pAsservEsialR_ = NULL;
...
void Asserv::setLowSpeedForward(bool enable, int percent) {
    logger().error() << "Asserv::setLowSpeedForward Surcharge à faire par config Robot!!!!!!!! percent " << percent << logs::end;
    if (useAsservType_ == ASSERV_INT_INSA) {
    ...
    else if (useAsservType_ == 0) asservdriver_->motion_setLowSpeedForward(enable, percent);
}

void Asserv::setLowSpeedBackward(bool enable, int percent) {
    logger().error() << "Asserv::setLowSpeedBackward Surcharge à faire par config Robot!!!!!!!! percent " << percent << logs::end;
    if (useAsservType_ == ASSERV_INT_INSA) {
        logger().error() << "TODO setLowSpeed ASSERV_INT_INSA NOT EXIST!!!" << logs::end;
    }
    else if (useAsservType_ == ASSERV_INT_ESIALR) {
    ...
    else if (useAsservType_ == 0) asservdriver_->motion_setLowSpeedBackward(enable, percent);
}
...
void Asserv::freeMotion() {
    if (useAsservType_ == ASSERV_INT_INSA) pAsservInsa_->motion_FreeMotion();
    else if (useAsservType_ == ASSERV_EXT) asservdriver_->motion_FreeMotion();
    else if (useAsservType_ == ASSERV_INT_ESIALR) pAsservEsialR_->motion_FreeMotion();
}

void Asserv::assistedHandling() {
    if (useAsservType_ == ASSERV_INT_INSA) pAsservInsa_->motion_AssistedHandling();
    else if (useAsservType_ == ASSERV_EXT) asservdriver_->motion_AssistedHandling();
...
void Asserv::setPositionAndColor(float x_mm, float y_mm, float thetaInDegrees_, bool matchColor = 0) {
    ...
    if (useAsservType_ == ASSERV_INT_INSA) pAsservInsa_->odo_SetPosition(x_mm / 1000.0, y_mm / 1000.0, thetaInDegrees * M_PI / 180.0);
    else if (useAsservType_ == ASSERV_EXT) asservdriver_->odo_SetPosition(x_mm, y_mm, thetaInDegrees * M_PI / 180.0);
    else if (useAsservType_ == ASSERV_INT_ESIALR) pAsservEsialR_->odo_SetPosition(x_mm, y_mm, thetaInDegrees * M_PI / 180.0);
}

void Asserv::setPositionReal(float x_mm, float y_mm, float thetaInRad) {
    if (useAsservType_ == ASSERV_INT_INSA) pAsservInsa_->odo_SetPosition(x_mm / 1000.0, y_mm / 1000.0, thetaInRad);
    else if (useAsservType_ == ASSERV_EXT) asservdriver_->odo_SetPosition(x_mm, y_mm, thetaInRad);
...
RobotPosition Asserv::pos_getPosition() {
    ...
    if (useAsservType_ == ASSERV_INT_INSA) p = pAsservInsa_->odo_GetPosition();
    else if (useAsservType_ == ASSERV_EXT) p = asservdriver_->odo_GetPosition();
...
void Asserv::resetEmergencyOnTraj(std::string message) {
    ...
    else if (useAsservType_ == ASSERV_EXT) asservdriver_->path_ResetEmergencyStop();
...
void Asserv::warnFrontCollisionOnTraj(int frontlevel, float x_adv_detect_mm, float y_adv_detect_mm) {
    ...
    if (useAsservType_ == ASSERV_INT_INSA) pAsservInsa_->path_CollisionOnTrajectory();
    else if (useAsservType_ == ASSERV_EXT) asservdriver_->path_CollisionOnTrajectory();
...
//conversion de la position du le terrain et determination du centre du robot adverse
    ...
    if (x_adv_detect_mm >= 1.0 || y_adv_detect_mm > 1.0) {
        ...
        x_adv = p_current.x + ((x_adv_detect_mm + dist_centre_robot_mm) * cos(p_current.theta)) - (y_adv_detect_mm * sin(p_current.theta));
        y_adv = p_current.y + ((x_adv_detect_mm + dist_centre_robot_mm) * sin(p_current.theta)) + (y_adv_detect_mm * cos(p_current.theta));
...
void Asserv::warnBackCollisionOnTraj(float x_adv_detect_mm, float y_adv_detect_mm) //x positif devant le robot, y positif le coté gauche
{
    ...
    if (useAsservType_ == ASSERV_INT_INSA) pAsservInsa_->path_CollisionRearOnTrajectory();
    else if (useAsservType_ == ASSERV_EXT) asservdriver_->path_CollisionRearOnTrajectory();
...
//conversion de la position du le terrain et determination du centre du robot adverse
    ...
    logger().info() << "warnBackCollisionOnTraj x_adv = "
            << x_adv
            << " y_adv = "
            << y_adv
            << "         x_adv_detect_mm = "
...
TRAJ_STATE Asserv::doLineAbs(float dist_mm) // if distance <0, move backward
{
    ...
    if (useAsservType_ == ASSERV_INT_INSA) ts = pAsservInsa_->motion_DoLine(dist_mm / 1000.0f);
    else if (useAsservType_ == ASSERV_EXT) ts = asservdriver_->motion_DoLine(dist_mm);
    else if (useAsservType_ == ASSERV_INT_ESIALR) ts = pAsservEsialR_->motion_DoLine(dist_mm);
    else ts = TRAJ_ERROR;
...
TRAJ_STATE Asserv::doRotateAbs(float degreesRelative) {
    ...
    if (useAsservType_ == ASSERV_INT_INSA) ts = pAsservInsa_->motion_DoRotate(radians);
    else if (useAsservType_ == ASSERV_EXT) ts = asservdriver_->motion_DoRotate(radians);
    else if (useAsservType_ == ASSERV_INT_ESIALR) ts = pAsservEsialR_->motion_DoRotate(radians);
    else ts = TRAJ_ERROR;
...
TRAJ_STATE Asserv::doMoveForwardTo(float xMM, float yMM, bool rotate_ignored, float adjustment_mm) {
    ...
    logger().info() << "doMoveForwardTo doRotateTo degrees="
            ...
            << (aRadian * 180.0f) / M_PI
            << " get="
            << getRelativeAngle((aRadian * 180.0f) / M_PI)
            << " xMM="
            << xMM
            << " yMM="
            << yMM
...
std::tuple<int, float, float> Asserv::eq_2CirclesCrossed_getXY(float x1, float y1, float d1, float x2, float y2, float d2, float robot_size_l_mm) {
    logger().debug() << "eq_2CirclesCrossed_getXY x1="
            << x1
            << " y1="
            << y1
            << " d1="
...
int Asserv::adjustRealPosition(float pos_x_start_mm, float pos_y_start_mm, RobotPosition p, float delta_jx_mm, float delta_ky_mm, float mesure_mm,
        float robot_size_l_mm)
{

    float pos_x_start_mm_conv = getRelativeX(pos_x_start_mm);
    logger().debug() << "adjustRealPosition : pos_x_start_mm="
            << pos_x_start_mm_conv
            << " pos_y_start_mm="
            << pos_y_start_mm
            << " p.x="
    ...
    float dist_real_mm = std::sqrt(
            (((p.x) - pos_x_start_mm_conv) * ((p.x) - pos_x_start_mm_conv)) + (((p.y) - pos_y_start_mm) * ((p.y) - pos_y_start_mm)));
...
//on determine le croisement des 2 cercles en fct des coord et des rayons
    ...
    logger().debug() << "alphap_rad="
            ...
            << " deg="
            << angle_rad * 180 / M_PI
            << " position_rel_theta_when_mesuring_rad="
            << position_rel_theta_when_mesuring_rad
            << " dist_x_when_mesuring_mm="
            << dist_x_when_mesuring_mm
            << " dist_real_mm="
            << dist_real_mm
            << " BCprim="
...
bool Asserv::calculateDriftRightSideAndSetPos(float d2_theo_bordure_mm, float d2b_bordure_mm, float x_depart_mm, float y_depart_mm) {
...
//partie reelle (avec la distance mesurée de la bordure
        ...
        logger().error() << "old position : " << " x= " << p.x << " y= " << p.y << " a_deg= " << p.theta * 180.0 / M_PI << logs::end;
...
bool Asserv::calculateDriftLeftSideAndSetPos(float d2_theo_bordure_mm, float d2b_bordure_mm, float x_depart_mm, float y_depart_mm) {
    logger().error() << "calculate : "
            << " d2_theo_bordure_mm= "
            << d2_theo_bordure_mm
            << " d2b_bordure_mm= "
...
TRAJ_STATE Asserv::doCalage2(int distmm, int percent) {
    if (useAsservType_ == ASSERV_INT_ESIALR) {
        if (distmm > 0) pAsservEsialR_->motion_setLowSpeedForward(true, percent);
...
TRAJ_STATE Asserv::doCalage(int distmm, int percent) {
    ...
    if (useAsservType_ == ASSERV_INT_ESIALR) {
        ...
        if (distmm > 0) asservdriver_->motion_setLowSpeedForward(true, percent);
