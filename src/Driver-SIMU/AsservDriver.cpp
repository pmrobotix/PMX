...
{
	//usleep(2000);//pour laisser le temps à l'initialisation du svg et du debut du fichier.
	int periodTime_us = 30000;
	...
	while (1)
	{
		if (asservStarted_)
		{
			...
			loggerSvg().info() << "<circle cx=\"" << p.x * 1000.0 << "\" cy=\"" << -p.y * 1000.0
					<< "\" r=\"1\" fill=\"blue\" />"
					<< "<line x1=\""<< p.x * 1000.0 << "\" y1=\"" << -p.y * 1000.0 << "\" x2=\""
					<< p.x * 1000.0 + cos(p.theta) * 25
					<< "\" y2=\""
					<< -p.y * 1000.0 -sin(p.theta) * 25
...
TRAJ_STATE AsservDriver::motion_DoLine(float dist_meters)
{
	...
	if (abs(deltaXm) < 0.0001) deltaXm = 0;

	double deltaYm = sin(t_init) * dist_meters;
	if (abs(deltaYm) < 0.0001) deltaYm = 0;
	...
	logger().debug() << "dist_meters=" << dist_meters << "deltaXm=" << deltaXm << " deltaYm="
			<< deltaYm << " a=" << a << " b=" << b << "  !!!!!" << logs::end;
	...
	logger().debug() << "tps=" << tps_sec << " increment_m=" << increment_m << "  !!!!!"
			<< logs::end;
