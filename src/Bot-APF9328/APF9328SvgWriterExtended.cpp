...
/*
//angle en radian
void APF9328SvgWriterExtended::writePosition(double x, double y, double angle_rad, std::string symbol)
{
    if (logger().isActive(logs::Level::INFO)) {
        ...
        if (symbol == "bot") {
            logger().info() << "<use x=\"" << x - 150 << "\" y=\"" << -y - 150
                    << "\" xlink:href=\"#bot-apf9328\" transform=\"rotate(" << -((angle_rad * 180) / M_PI) << "," << x
                    << "," << -y << ")\" />" << logs::end;
...
void APF9328SvgWriterExtended::writePosition_Bot(float x, float y, float a_rad)
{
    logger().info() << "<use x=\"" << x - 150 << "\" y=\"" << -y - 150
            << "\" xlink:href=\"#bot-apf9328\" transform=\"rotate(" << -((a_rad * 180) / M_PI) << "," << x << ","
            << -y << ")\" />" << logs::end;
}
void APF9328SvgWriterExtended::writePosition_BotPos(float x, float y, float a_rad)
{
    double delta_y = 0.0;
    double delta_x = 0.0;
    // inversion du y pour affichage dans le bon sens dans le SVG
    logger().info() << "<circle cx=\"" << x << "\" cy=\"" << -y << "\" r=\"1\" fill=\"blue\" />" << logs::end;
    delta_y = 50.0 * sin(a_rad);
    delta_x = 50.0 * cos(a_rad);
    logger().info() << "<line x1=\"" << x << "\" y1=\"" << -y << "\" x2=\"" << x + delta_x << "\" y2=\"" << -y - delta_y
            << "\" stroke-width=\"0.1\" stroke=\"grey\"  />" << logs::end;
}

void APF9328SvgWriterExtended::writeZone(const char* name, float minX, float minY, float width, float height,
...
void APF9328SvgWriterExtended::writeIaPath(const char* zone1Name, const char* zone2Name, float x_mm, float y_mm)
{
