...
void L_MovingBaseTest::run(int argc, char** argv)
{
	...
	robot.asserv().setPositionAndColor(0.0, 300.0, 0.0, (robot.getMyColor() != PMXORANGE));
	...
	if (LR == 0 || LR == 2)
		robot.asserv().base()->motors().runMotorLeft(p, 2000); //run (power 128) pendant 1s
	if (LR == 1 || LR == 2)
		robot.asserv().base()->motors().runMotorRight(p, 2000); //run (power 128) pendant 1s
	chrono.start();
	...
	logger().info() << "runMotor(-" << p << ", 1000) " << logs::end;
	if (LR == 0 || LR == 2)
		robot.asserv().base()->motors().runMotorLeft(-p, 2000); //run with power -100 pendant 1s
	if (LR == 1 || LR == 2)
		robot.asserv().base()->motors().runMotorRight(-p, 2000); //run with power -100 pendant 1s
	chrono.start();
	while (chrono.getElapsedTimeInMilliSec() < 2500.0)
	{
		left = robot.asserv().base()->encoders().getLeftEncoder();
