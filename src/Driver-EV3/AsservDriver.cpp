...
AsservDriver::AsservDriver()
{
	...
	for (unsigned i = 0; i < 4; ++i)
	{
		...
		if (m.connected())
		{
			...
			logger().info() << "(" << 1 + i << ") " << m.driver_name() << " motor on port "
					<< m.address() << logs::end;
		}
...
//test3
	if (_motor_left.connected())
	{
		...
		logger().info() << "(" << 1 << ") " << _motor_left.driver_name() << " motor on port "
				<< _motor_left.address() << logs::end;
	}

	if (_motor_right.connected())
	{

		logger().info() << "(" << 4 << ") " << _motor_right.driver_name() << " motor on port "
				<< _motor_right.address() << logs::end;
	}
