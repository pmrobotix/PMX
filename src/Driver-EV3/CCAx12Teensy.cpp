...
CCAx12Teensy::CCAx12Teensy() : //TODO Une SEULE CARTE est possible a mettre en paramettre adresse + port mais attention à l'instance unique
        i2c_CCAx12Teensy_(2, true), connected_(false)
{
    i2c_aAddr_ = 0;
}
...
/*
     i2cHandle_ = open("/dev/i2c-in2", O_RDWR);
     if (i2cHandle_ == -1) {
     fprintf(stderr, "initI2C: failed to open: %s\n", strerror(errno));
     return -1;
     }

     int err = ioctl(i2cHandle_, I2C_SLAVE, i2c_aAddr);
     if (err < 0) {
     fprintf(stderr, "ioctl: failed to ioctl: %s i2cHandle_:%d i2c_aAddr:%d\n", strerror(errno), i2cHandle_, i2c_aAddr);
     return -1;
     }

     printf("i2cHandle_:%d i2c_aAddr_:%d\n", i2cHandle_, i2c_aAddr_);
     int port = 1;
     int id = 8;
     while (1) {
     //        int erro = setLedOn(5);
     //        if (erro < 0) printf("setLedOn erro:%d\n", erro);
...
*/
    ...
    if (err >= 0) {
        ...
        while (1) {
            ...
            int erro = setLedOn(5);
            if (erro < 0) printf("setLedOn erro:%d\n", erro);
            //logger().error() << "setLedOn erro=" << erro << logs::end;

            int port = 1;
            int id = 8;
//        int p = pingAX(port, id);
//            //
//                     printf("Testing AX on port %d, id %d, ping:%d \n", port, id, p);
//            printf("read AX %d MODEL_NUMBER returned %d\n", id, readAXData(port, id, P_MODEL_NUMBER));
...
//            usleep(200000);
//            erro = setLedOff(5);
//            if (erro < 0) printf("setLedOff erro:%d\n", erro);
//            //logger().error() << "setLedff erro=" << erro << logs::end;
            //
            usleep(200000);
