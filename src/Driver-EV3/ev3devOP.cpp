...
private:
    ...
    {
        return std::find_if(_items.begin(), _items.end(), [&](const item &i) {return i.first == k;});
    }
...
bool device::connect(const std::string &dir, const std::string &pattern,
...
int device::device_index() const
{
    ...
    if (_path.empty())
        throw system_error(make_error_code(errc::function_not_supported), "no device connected ; [device_index] ");
...
std::string device::get_attr_string(const std::string &name) const
{
    ...
    if (_path.empty())
        throw system_error(make_error_code(errc::function_not_supported),
                "no device connected ; [get_attr_string] path=" + _path + " " + name);
    ifstream &is = ifstream_open(_path + name);
    if (is.is_open()) {
        string result;
        is >> result;
...
void device::set_attr_string(const std::string &name, const std::string &value)
{
    ...
    for (int attempt = 0; attempt <= 10; ++attempt) {
        ...
        if (os.is_open()) {
            printf("[set_attr_string] open try attemp=%d  name=%s%s  value=%s\n", attempt, _path.c_str(), name.c_str(), value.c_str());
            if (os << value)
                return;
...
//             }
            ...
            throw system_error(std::error_code(errno, std::system_category()),
                    "[set_attr_string] (os.is_open error! attempt=" + std::to_string(attempt));
    }
...
std::string device::get_attr_line(const std::string &name) const
{
    ...
    if (_path.empty())
        throw system_error(make_error_code(errc::function_not_supported),
                "no device connected ; [get_attr_line] path=" + _path + " " + name);
...
std::string sensor::type_name() const
{
    ...
    static const std::map<sensor_type, const std::string> lookup_table { { ev3_touch, "EV3 touch" }, { ev3_color,
            "EV3 color" }, { ev3_ultrasonic, "EV3 ultrasonic" }, { ev3_gyro, "EV3 gyro" }, { ev3_infrared,
            "EV3 infrared" }, { nxt_touch, "NXT touch" }, { nxt_light, "NXT light" }, { nxt_sound, "NXT sound" }, {
            nxt_ultrasonic, "NXT ultrasonic" }, { nxt_i2c_sensor, "I2C sensor" }, };
...
button::button(int bit) :
        _bit(bit), _buf((KEY_CNT + bits_per_long - 1) / bits_per_long), _fd(
                new file_descriptor("/dev/input/by-path/platform-gpio_keys-event", O_RDONLY))
{
}
...
{
    std::array<bool, 6> changed { { back.process(), left.process(), right.process(), up.process(), down.process(),
            enter.process() } };
...
void sound::tone(const std::vector<std::vector<float> > &sequence, bool bSynchronous)
{
...
void sound::speak(const std::string &text, bool bSynchronous)
{
    ...
    cmd << "/usr/bin/espeak -a 200 --stdout \"" << text << "\"" << " | /usr/bin/aplay -q";
...
void remote_control::on_value_changed(int value)
{
    ...
    if (((new_state & red_up) != (_state & red_up)) && static_cast<bool>(on_red_up))
        on_red_up(new_state & red_up);

    if (((new_state & red_down) != (_state & red_down)) && static_cast<bool>(on_red_down))
        on_red_down(new_state & red_down);

    if (((new_state & blue_up) != (_state & blue_up)) && static_cast<bool>(on_blue_up))
        on_blue_up(new_state & blue_up);

    if (((new_state & blue_down) != (_state & blue_down)) && static_cast<bool>(on_blue_down))
        on_blue_down(new_state & blue_down);

    if (((new_state & beacon) != (_state & beacon)) && static_cast<bool>(on_beacon))
        on_beacon(new_state & beacon);

    if ((new_state != _state) && static_cast<bool>(on_state_change))
