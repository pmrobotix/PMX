...
void GroveColorSensor::begin()
{
	try
	{
		//open i2c and setslave
		grovei2c_.setSlaveAddr(GROVE_COLOR_DEFAULT_ADDRESS);
		...
		{
			logger().error() << "init() : TCS3414 is now OFF, ID=" << (int) ID << " not eq 17 !" << logs::end;
		}
...
{
	logger().debug() << "conected=" << connected_ << logs::end;
	...
	if (connected_)
	{
		...
		{
			logger().error() << "Exception TSC3414Blue: " << e->what() << logs::end;
		}
	}
	return blueHigh;
}
...
unsigned char  GroveColorSensor::TSC3414Green()
{
	...
	if (connected_)
	{
		...
		{
			logger().error() << "Exception TSC3414Green: " << e->what() << logs::end;
		}
	}
	return greenHigh;
}
...
unsigned char  GroveColorSensor::TSC3414Red()
{
	...
	if (connected_)
	{
		...
		{
			logger().error() << "Exception TSC3414Red: " << e->what() << logs::end;
		}
	}
	return redHigh;
}
...
unsigned char GroveColorSensor::TSC3414Clear()
{
	...
	if (connected_)
	{
		...
		{
			logger().error() << "Exception TSC3414Clear : " << e->what() << logs::end;
		}
	}
	return clearHigh;
}
...
void GroveColorSensor::TCS3414Setup(int delay1, int delay2)
{
	...
	if (connected_)
	{
		...
		{
			logger().error() << "Exception GroveColorSensor::TCS3414Initialize : " << e->what() << logs::end;
		}
...
float GroveColorSensor::CCTCalc(unsigned char  allcolors[])
{
	...
	logger().debug() << "Illuminance: " << TCS3414tristimulus_[1] << " \tx: " << TCS3414chromaticityCoordinates_[0]
			<< " \ty: " << TCS3414chromaticityCoordinates_[1] << " \tCCT:  " << CCT << " K" << logs::end;
...
unsigned char * GroveColorSensor::TCS3414GetColor()
{
	...
	logger().info() << "Clear: " << (int) TCS3414values_[0] << " \tRed: " << (int) TCS3414values_[1] << " \tGreen: "
			<< (int) TCS3414values_[2] << " \tBlue:  " << (int) TCS3414values_[3] << logs::end;
...
void GroveColorSensor::write_i2c(unsigned char  command, unsigned char  value)
{
	//grovei2c_.writeRegValue(GROVE_COLOR_DEFAULT_ADDRESS, command, value);
	grovei2c_.writeRegByte(command, value);
}
