...
*/
		...
		case TRAJECTORY_RUNNING:
			...
			if (cap_enabled_ && cap_enabled_line)
			{
				...
				float dest = sqrt(dx * dx + dy * dy);
				logger().debug() << "cap dest=" << dest << " y_mm=" << y_mm
						<< " goal_pos_y_m="	<< goal_pos_y_m << logs::end;
				...
				else
				{
					...
					if (cap_theta_ > M_PI) cap_theta_ -= M_2PI;
					if (cap_theta_ < -M_PI) cap_theta_ += M_2PI;
...
//					<< logs::end;
		...
		logger().debug() << "loopDelayInMillis= " << loopDelayInMillis << " task duration us =\t"
				<< endTaskTime - currentTime << " usleep(nextTime-execTime) =\t"
				<< nextTime - endTaskTime << " nextTime =\t" << nextTime << logs::end;
