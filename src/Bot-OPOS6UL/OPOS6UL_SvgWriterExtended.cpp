...
//angle en radian
void OPOS6UL_SvgWriterExtended::writePosition(double x,
		double y,
		double angle_rad,
		std::string symbol)
{
	if (logger().isActive(logs::Level::INFO))
	{
		...
		if (symbol == "bot")
		{
			logger().info() << "<use x=\""
					...
					<< "\" y=\""
					<< -y - 150
					<< "\" xlink:href=\"#bot-OPOS6UL\" transform=\"rotate("
					<< -((angle_rad * 180) / M_PI)
					<< ","
					<< x
					<< ","
...
void OPOS6UL_SvgWriterExtended::writeZone(const char* name,
		float minX,
		float minY,
		float width,
		float height,
...
void OPOS6UL_SvgWriterExtended::writeIaPath(const char* zone1Name,
